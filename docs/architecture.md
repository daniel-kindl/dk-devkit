...
`~/.local/bin/agentqueue` is a devbox router shim, generated by
`devbox new-shim agentqueue --env web-dev` and pinned to the environment that
...
its own toolchain lives. `agentqueue` is installed in one environment only, so
