...
# public entry points first, so a deleted ~/.local/bin/agentbox or
# ~/.local/bin/agent-sandbox is repaired by the next run instead of being
# hidden by a healthy runtime.
#
...
for name in agentbox agent-sandbox; do
    command=$HOME/.local/bin/$name
    [ -x "$command" ] ||
        { echo "the $name command is not installed in ~/.local/bin" >&2; exit 1; }
    [ "$command" -ef "$REPO_ROOT/bin/$name" ] ||
        { echo "the installed $name command is not this checkout" >&2; exit 1; }
done
exec "$HOME/.local/bin/agentbox" doctor
